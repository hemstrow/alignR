...
alignR requires a suite of Unix-exclusive dependencies (SAMtools, ANGSD, and BWA) as well as both perl and a bash terminal in order to execute most of its functions. While installing these is pretty straight-forward on a Linux or MacOS-based machine, it's quite a bit more complicated on a Windows OS. Here, we'll walk through installing Windows Subsystem for Linux (wsl), R, Rstudio server, and the alignR dependencies on Windows. **Note that this requires Windows 10 Build 19041 and higher.**
...
From your Rstudio server session, install the remotes package:

```{r}
install.packages("remotes")
```

Remember that this is a separate R install, and that you will need to re-install any packages that you may have previously installed on your primary R install.
...
Your WSL will definitely have bash installed, and very likely have perl installed as well. If you don't have perl, you can install it by running:
...
Note that this will leave a few relics in your directory that you can remove if you so desire.
