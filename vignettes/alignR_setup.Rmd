...
## ngsParalog
alignR only needs ngsParalog if you are planning to do paralog filtering during genotype calling with `genotype_bams()`. Current versions of `ngsParalog` unfortunately are failing to compile on most systems (hopefully this will be fixed soon!). For now, a compiled version can be downloaded from X. After downloading, move the file to somewhere on your system `$PATH`:
